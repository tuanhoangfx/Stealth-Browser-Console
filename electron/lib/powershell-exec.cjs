 ...
 * Electron's fs.existsSync returns true for asar virtual paths, so prefer asar.unpacked first.
 */
function resolveElectronLibScript(fileName, baseDir = __dirname) {
  const local = path.join(baseDir, fileName);
  const asarNeedle = `${path.sep}app.asar${path.sep}`;
  const unpackedNeedle = `${path.sep}app.asar.unpacked${path.sep}`;
  if (local.includes(asarNeedle) && !local.includes(unpackedNeedle)) {
    const unpacked = local.replace(asarNeedle, unpackedNeedle);
    if (fs.existsSync(unpacked)) return unpacked;
  }
  if (fs.existsSync(local) && !local.includes(asarNeedle)) return local;
  if (fs.existsSync(local) && local.includes(unpackedNeedle)) return local;
  return local.includes(asarNeedle) && !local.includes(unpackedNeedle)
    ? local.replace(asarNeedle, unpackedNeedle)
    : local;
}
